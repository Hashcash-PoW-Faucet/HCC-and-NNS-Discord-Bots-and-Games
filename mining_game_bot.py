...
def main():
    ...
    def update_names_cache_for_user(user: discord.abc.User) -> None:
        ...
        try:
            uid = str(int(user.id))
            # Prefer guild display name (nickname) when available; otherwise global name/username
            name = getattr(user, "display_name", None) or getattr(user, "global_name", None) or getattr(user, "name",
                                                                                                        None) or uid
            name = str(name).strip() or uid
            cache = _load_names_cache()
            if cache.get(uid) != name:
    ...
    class MinerView(View):
        ...
        class BuyAsicButton(Button):
            ...
            async def callback(self, interaction: discord.Interaction):
                ...
                if not ok:
                    await interaction.response.send_message(
                        "❌ Still processing the previous action or insufficient funds.", ephemeral=True)
                    return
                rig.setdefault("asics", []).append({"stars": 1, "upgrade_ready_time": None, "overclock_until": None})
                view.miner_bot.update_user_rig(view.user_id, rig)
                await interaction.response.send_message("𒁈 ASIC bought and added to your rig!", ephemeral=True)
        ...
        class OverclockAsicButton(Button):
            ...
            async def callback(self, interaction: discord.Interaction):
                ...
                dur = int(overclock_duration_seconds_for_rig(rig))
                await interaction.response.send_message(
                    f"Select an ASIC to overclock ({fmt_duration_days_only(dur)} boost):", view=oc_view, ephemeral=True)
        ...
        class OverclockGpuButton(Button):
            ...
            async def callback(self, interaction: discord.Interaction):
                ...
                dur = int(overclock_duration_seconds_for_rig(rig))
                await interaction.response.send_message(
                    f"Select a GPU to overclock ({fmt_duration_days_only(dur)} boost):", view=oc_view, ephemeral=True)
        ...
        class OverclockAllAsicsButton(Button):
            ...
            async def callback(self, interaction: discord.Interaction):
                ...
                if GAME_TREASURY_DISCORD_ID == 0:
                    await interaction.response.send_message("⚠️ Treasury not configured. Cannot overclock.",
                                                            ephemeral=True)
                    return
        ...
        def __init__(self, user_id, miner_bot, requester=None):
            ...
            if discord.utils.get(self.children, custom_id="refresh") is None:
                # Only the owner (or the original requester) should get interactive controls
                print(
                    f"[MinerView] building view for user={self.user_id} children_before={len(self.children)}")  # Debug
                if requester is None or requester.id == self.user_id:
                    rig = self.miner_bot.get_user_rig(self.user_id)
                    if not rig or not isinstance(rig, dict):
                        rig = {"rig_level": 1, "asics": [], "gpus": [], "upgrade_ready_time": None,
                               "power_plant_level": 0}
                    ...
                    if ppl < int(POWER_PLANT_MAX_LEVEL):
                        ...
                        self.add_item(self.UpgradePowerPlantButton(ppl, nxt, cost_pp, oc_dur_next, row=4))
                    print(
                        f"[MinerView] built view for user={self.user_id} total_children={len(self.children)}")  # Debug
            ...
            async def callback(self, interaction: discord.Interaction):
                try:
                    ...
                    if balance < self.upgrade_cost:
                        ...
                        return
                    if GAME_TREASURY_DISCORD_ID == 0:
                        await interaction.response.send_message("⚠️ Treasury not configured. Cannot upgrade.",
                                                                ephemeral=True)
                        return
                    ok = transfer_internal(view.user_id, GAME_TREASURY_DISCORD_ID, self.upgrade_cost,
                                           note="upgrade rig")
                    if not ok:
                        await interaction.response.send_message(
                            "❌ Still processing the previous action or insufficient funds.", ephemeral=True)
        ...
        class UpgradePowerPlantButton(Button):
            ...
            async def callback(self, interaction: discord.Interaction):
                ...
                bal = get_user_balance(view.user_id)
                if bal < cost:
                    await interaction.response.send_message("❌ Not enough Ħ to upgrade the Power Plant.",
                                                            ephemeral=True)
                    return
    ...
    class MinerGameBot:
        def build_help_embed(self, user: 'discord.abc.User | None' = None) -> 'discord.Embed':
            ...
            if user is not None:
                embed.set_author(name=str(
                    getattr(user, "display_name", None) or getattr(user, "name", None) or getattr(user, "id", "")))
            return embed
        ...
        def __init__(self):
            load_dotenv()
            self.MINERGAME_CHANNELS = [int(x.strip()) for x in os.getenv("MINERGAME_CHANNEL_IDS", "").split(",") if
                                       x.strip()]
            # Use non-privileged intents only (no Members / Presence / Message Content).
            # Slash commands do not require privileged intents.
            intents = discord.Intents.default()
                ...
                if not rig:
                    ...
                    class BuildRigButton(discord.ui.Button):
                        ...
                        async def callback(self, interaction: discord.Interaction):
                            balance = get_user_balance(interaction.user.id)
                            if balance < RIG_BASE_BUILD_COST:
                                await interaction.response.send_message("❌ Not enough Ħ to build a miner rig.",
                                                                        ephemeral=True)
                                return
                            if GAME_TREASURY_DISCORD_ID == 0:
                                await interaction.response.send_message("⚠️ Treasury not configured. Cannot build rig.",
                                                                        ephemeral=True)
                                return
                            ok = transfer_internal(interaction.user.id, GAME_TREASURY_DISCORD_ID, RIG_BASE_BUILD_COST,
                                                   note="Initial Miner Rig Build")
                            if not ok:
                                await interaction.response.send_message(
                                    "⚠️ Transaction already pending or insufficient funds. Please wait.",
                                    ephemeral=True)
                                return
                            now = int(time.time())
                            new_rig = {
                    ...
                    view.add_item(BuildRigButton(self))
                    await interaction.followup.send("You don't own a miner rig yet. Would you like to build one?",
        ...
        def create_miner_embed(rig, user_id):
            ...
            try:
                ...
                total_weight = float(sum(weights.values()))
                if total_weight > 0:
                    treasury_bal = int(get_user_balance(GAME_TREASURY_DISCORD_ID)) if int(
                        GAME_TREASURY_DISCORD_ID) != 0 else 0
                    payout_total = payout_total_for_state({"users": snap_users})
                    est_total_to_pay = int(min(int(payout_total), int(treasury_bal)))
    ...
    def main():
        ...
        if GAME_TREASURY_DISCORD_ID == 0:
            print(
                "⚠️  GAME_TREASURY_DISCORD_ID is 0. Buying/upgrades/payouts will be disabled until you set it in .env")
        try:
            with open(FACTORY_FILE, "r", encoding="utf-8") as f:
